...
###import nltk build-in stopwords
from nltk.corpus import stopwords
stops = set(stopwords.words("english"))
from wordcorrection import *
...
def str_stem(s): 
    if isinstance(s, str):
        ...
        wordlem = [wnl.lemmatize(z) for z in tokens if z not in stop_w]
        ###correction
       # wordcor = [correct(w) for w in wordlem]
        ###stemmer
        s = (" ").join([stemmer.stem(z) for z in wordlem])
        return s
    else:
        return "null"
...
clf = pipeline.Pipeline([
        ...
        ('rfr', rfr)])
param_grid = {'rfr__max_features': [15], 'rfr__max_depth': [15]}
...
pd.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('submissionnlk.csv',index=False)
